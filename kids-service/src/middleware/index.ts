...
const authMiddleware = async (
    ...
    const token = authHeader?.split(" ")[1] || req.cookies?.jwt; // Check both header and cookie
    console.log("Cookies:🍪🍪🍪🍪", req.cookies);
    if (!token) {
        return next(new ApiError(401, "Missing token in authorization header or cookie"));
    }
